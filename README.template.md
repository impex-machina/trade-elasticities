...
| `sigma_robust` | Cell-level (`importer × good`) flag: `TRUE` when γ's SE is robust to Stage-1 σ uncertainty — σ̂ is unclamped, has a finite SE, sits clear of the σ = 1 identification pole, and the propagated term inflates no γ SE in the cell beyond the screen threshold. `FALSE` when any of those fail; `NA` for Tier-3 imputed cells with no per-cell σ. `TRUE` on {{format_pct(req(r$stage2b$sigma_robust, "robust"), req(r$stage2b$sigma_robust, "total"))}} of rows ({{format_pct(req(r$stage2b$sigma_robust, "robust"), req(r$stage2b$sigma_robust, "n_estimated"))}} of the rows outside Tier 3), `FALSE` on {{format_pct(req(r$stage2b$sigma_robust, "not_robust"), req(r$stage2b$sigma_robust, "total"))}}, `NA` on {{format_pct(req(r$stage2b$sigma_robust, "tier3_na"), req(r$stage2b$sigma_robust, "total"))}}. Filter on this to keep only cells whose γ SE is stable once σ is treated as estimated rather than known (see Known limitations). |
...
│   └── manifest.csv   # the {{manifest_n_files}} published data files, with SHA-256 checksums
...
- **γ heterogeneity is estimated for large exporters and imputed for the tail.** The Stage-2 log-ridge (λ = 0.1) is strong relative to the data curvature of a time-averaged moment row: the median row keeps a data share of {{format_num(req(r$stage2_shrinkage_census_v074rc_sandwich$q1$implied_data_share, "p50"))}}, the trade-weighted share is {{format_num(req(r$stage2_shrinkage_census_v074rc_sandwich$q1, "implied_data_share_trade_weighted"))}}, and a cell's largest exporter keeps {{format_num(req(Filter(function(b) identical(b$rank_bin, "1"), r$stage2_shrinkage_census_v074rc_sandwich$q1$by_trade_rank)[[1]], "data_share_median"))}} against {{format_num(req(Filter(function(b) identical(b$rank_bin, ">10"), r$stage2_shrinkage_census_v074rc_sandwich$q1$by_trade_rank)[[1]], "data_share_median"))}} for exporters ranked below tenth. Exporter-within-cell variation is {{format_pct(100 * req(r$stage2_shrinkage_census_v074rc_sandwich$q2$trade_weighted, "share_exporter_within_cell"), 100)}} of the trade-weighted variance of log γ. A full-universe λ curve (`docs/results/stage2_lambda_curve_v074rc.md`) shows the tails exploding below λ = 0.01 while the median row barely moves, so λ stays at 0.1; the prior's form (a prior on log γ cannot represent near-perfectly-elastic supply, γ ≈ 0), the measurement-error correction and the reference-exporter export moment are open v0.8.0 items. Details: `docs/methodology/stage2_country.md` ("Log-ridge domain", "Variance formula").
...
- **Standard errors: conditional on σ, with a robustness screen.** `gamma_se`
  ...
  {{format_pct(req(r$stage2b$se_status, "other"), req(r$stage2b$se_status, "total"))}} are boundary, plateau, non-converged, or unflagged fits. The pipeline
  ...
  cell inflated beyond threshold. {{format_pct(req(r$stage2b$sigma_robust, "robust"), req(r$stage2b$sigma_robust, "total"))}} of rows pass ({{format_pct(req(r$stage2b$sigma_robust, "robust"), req(r$stage2b$sigma_robust, "n_estimated"))}} of the rows outside Tier 3); {{format_pct(req(r$stage2b$sigma_robust, "not_robust"), req(r$stage2b$sigma_robust, "total"))}} are flagged `FALSE` and {{format_pct(req(r$stage2b$sigma_robust, "tier3_na"), req(r$stage2b$sigma_robust, "total"))}} are Tier-3 imputed cells (`NA`) with no per-cell σ. The screen is governed almost entirely by σ̂'s distance from the σ = 1 pole, not by the inflation cutoff, and the pass rate is stable across a wide grid of both thresholds (`analysis/sensitivity_sweep.R` reproduces it). Where `sigma_robust` is `FALSE` or `NA`, treat `gamma_se` as a conditional, lower-bound measure of uncertainty; where `TRUE`, `gamma_se_total` is the σ-aware SE — and in either case the SE is frequently as large as the estimate itself.{{step2_vce_clause(r$stage1$step2_vce)}}{{bootstrap_clause(r$bootstrap_se)}}
