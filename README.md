...
producing 6,860,437 (importer, exporter, HS4) cell-level γ estimates with
...
185,144 of them (one importer present at Stage 1 has no country-pair γ at Stage 2b after the minimum-destinations filter).
...
| `sigma` | Import-demand (substitution) elasticity for the product, fixed from Stage 1 and constant within a product. For 10.0% of rows this is a global-median fallback (σ ≈ 2.705) rather than a cell-specific estimate, and a further 10.1% sit at the cap value of 10 (see Known limitations). |
| `gamma` | **Inverse export-supply elasticity** for the (importer, exporter, product) cell — the headline estimate, Soderbery's γ. Lower-bounded near 0 by the optimizer and **unbounded above** (extreme values are handled by the Stage 2a plateau fallback and the 0.5%-per-tail trim; see Known limitations). γ is the *inverse* of an elasticity: the implied export-supply elasticity is 1 / γ (median ≈ 1.522). Small γ means near-perfectly-elastic supply; large γ means strong importer market power. Most cells are shrunk toward a good-level prior (see Known limitations). |
...
A reader reproducing the headline numbers should find σ median ≈ 2.705 on
...
- **σ small-sample bias and selection under the Stage-1 homogeneity
  ...
  an estimate* in 40.5%–95.5% of replications across the σ × ω grid
  (median 71.8%) — a yield rate, not a recovery-within-tolerance rate —
  ...
  -46.2% to 5.6% (negative at 9 of 12 grid points),
  so comparisons to Feenstra-GMM or Broda–Weinstein estimates should not
  assume the upward bias of that tradition.
- **Estimator-provenance composition.** On the full universe, 41.1% of
  (importer, HS4) cells are identified at the HLIML interior; the rest fall
  to the Step 2 fallback, of which 5.2% of the full universe (14,619 cells)
  are clamped at the σ/ω caps and report the cap, not an estimate; a further 10.7% (30,056 cells) are constrained boundary HLIML optima -- 13,893 on the ω floor, 9,152 at the σ cap, 7,011 at the ω cap -- routed only where both the closed-form HLIML point and Step 2 were inadmissible (`final_source == "hliml_boundary"`, no SE). 93.7% of cells fail the
  ...
  25% rule of thumb, 38.3% of evaluated cells pass the
  weak-instrument screen, 58.2% pass the Sargan
  overidentification test (conventional p > 0.2), and 22.7% pass both --
  ...
  the interior rate rises to 62.4%; both framings appear in the methodology
...
- **Supply-side flooring, and shrinkage that masks it.** The Stage 1
  ...
  Stage 1 ω-floor share is far larger, at 23.9%
...
- **Standard errors: conditional on σ, with a robustness screen.** `gamma_se`
  ...
  61.8% of rows carry a clean cell-specific SE: 25.7% are Tier 3 cells
  assigned the regional prior outright (no SE) and the remaining
  12.4% are boundary, plateau, non-converged, or unflagged fits. The pipeline
