...
producing 6,811,822 (importer, exporter, HS4) cell-level γ estimates with
...
| `gamma` | **Inverse export-supply elasticity** for the (importer, exporter, product) cell — the headline estimate, Soderbery's γ. Lower-bounded near 0 by the optimizer and **unbounded above** (extreme values are handled by the Stage 2a plateau fallback and the 0.5%-per-tail trim; see Known limitations). γ is the *inverse* of an elasticity: the implied export-supply elasticity is 1 / γ (median ≈ 1.503). Small γ means near-perfectly-elastic supply; large γ means strong importer market power. Most rows are shrunk toward a good-level prior — median `gamma_shrink_wt` 0.928, trade-weighted data share 0.227; see Known limitations. Since v0.7.4 the log-ridge applies to every coordinate, so no directly estimated row sits at the optimizer floor (v0.7.3 had 16.6% there). |
...
- **σ small-sample bias and selection under the Stage-1 homogeneity
  ...
  an estimate* in 80.0%–100.0% of replications across the σ × ω grid
  (median 98.0%) — a yield rate, not a recovery-within-tolerance rate —
...
- **Supply-side boundaries: two ends of the ω axis.** The Feenstra
  ...
  same through v0.6.1. From v0.7.0
...
- **Standard errors: conditional on σ, with a robustness screen.** `gamma_se`
  ...
  63.0% of rows carry a clean cell-specific SE: 26.7% are Tier 3 cells
  assigned the regional prior outright (no SE) and the remaining
  10.2% are boundary, plateau, non-converged, or unflagged fits. The pipeline
