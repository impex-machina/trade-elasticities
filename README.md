...
producing 6,831,219 (importer, exporter, HS4) cell-level γ estimates with
...
- **Estimator-provenance composition.** On the full universe, 18.2% of
  ...
  are clamped at the σ/ω caps and report the cap, not an estimate. 93.9% of cells fail the
  ...
  25% rule of thumb, 41.2% of evaluated cells pass the
  weak-instrument screen, 56.3% pass the Sargan
  overidentification test (conventional p > 0.2), and 22.4% pass both --
...
- **Standard errors: conditional on σ, with a robustness screen.** `gamma_se`
  ...
  65.0% of rows carry a clean cell-specific SE: 25.7% are Tier 3 cells
