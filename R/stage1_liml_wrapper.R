  ...
  process_one_cell <- function(idx, cell_data, imp, gd) {
    ...
    if (!isTRUE(fit$status == "ok"))
      return(list(importer = imp, good = gd, status = fit$status,
                  ...
                  step2_vce_method = fit$step2_vce_method %||% step2_vce,   # patch 0061
    ...
    list(
      ...
      stockyogo_pass_gs25 = fit$stockyogo_pass_gs25,
      stockyogo_cv_gs25 = fit$stockyogo_cv_gs25,
      sargan_pass = fit$sargan_pass,
      jstat_h_pval_gs = fit$jstat_h_pval_gs,
      sargan_pass_gs = fit$sargan_pass_gs,
      gs_pass_both = fit$gs_pass_both,
      adjust = fit$adjust,
  ...
  if (.Platform$OS.type == "windows" && n_cores > 1L) {
    ...
    parallel::clusterExport(cl,
                            c("min_year", "min_exporters", "min_periods", "process_one_cell",
                              ...
                              "uv_outlier_threshold", "step2_vce", "%||%"),
                            envir = environment())
